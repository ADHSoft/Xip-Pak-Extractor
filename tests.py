...
def tests(): 
    try:
        ...
        ans = main.xipRsa1(int(dataInput), int(key2), int(key1))
        assert hex(ans) == "0xab1102"

        ans = main.xipRsa2(1, int(dataInput), int(key1))
        ...
        ans=main.xipRsa(dataInput,0x0c)
        assert hex(int.from_bytes(ans, byteorder='big')) == "0x211ab0000007e000000012a"

        import xipDecoder_strategy

        assert hashlib.md5(xipDecoder_strategy.XipDecoderStrategy().japaneseTextXorEncoder()).hexdigest() == '74615d959b2d008869dc8fd289686702'
